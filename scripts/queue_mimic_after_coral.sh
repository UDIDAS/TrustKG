...
echo "$(date) CORAL ensemble complete -> launching MIMIC extraction (mimiciii on GPU0, mimiciv on GPU1)"
nohup "$PY" scripts/run_mimic_extraction.py --source mimiciii --gpu 0 > results/mimic_ex_gpu0.log 2>&1 &
nohup "$PY" scripts/run_mimic_extraction.py --source mimiciv  --gpu 1 > results/mimic_ex_gpu1.log 2>&1 &
wait
